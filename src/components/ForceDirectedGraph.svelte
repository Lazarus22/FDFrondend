<script>
  ...
  function renderVisualization(data) {
    ...
    const dynamicLinkDistance = Math.sqrt(nodes.length) * 5;
    const dynamicCharge = -Math.sqrt(nodes.length) * 5;
    ...
    const simulation = d3.forceSimulation(nodes)
                          .force('link', d3.forceLink(links).id(d => d.id).distance(dynamicLinkDistance))
    ...
    const node = container.append('g').attr('stroke', '#fff').attr('stroke-width', 1.5)
                  .selectAll('g').data(nodes).join('g')
                  .call(d3.drag().on('start', dragstarted).on('drag', dragged));

    node.append('circle').attr('r', 5).attr('fill', '#6699cc');
    node.append('text').attr('stroke', 'none').attr('fill', '#000')
